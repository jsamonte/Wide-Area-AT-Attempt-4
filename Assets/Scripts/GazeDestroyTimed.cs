...
public class DestructionRecord
{
    ...
    public float timeSinceAppStart;           // seconds since app launch
    public float timeSincePreviousDestroy;    // 0 for the first object
}

[System.Serializable]
public class DestructionSession
{
    public float timeToFirstDestroy;
    public float totalTimeToComplete;
    public int totalObjectsDestroyed;
    public List<DestructionRecord> destructionEvents = new List<DestructionRecord>();
}

public class GazeDestroyTimed : MonoBehaviour
{
    ...
    // === NEW: Destruction logging fields ===
    private DestructionSession sessionData;
    ...
    private void Awake()
    {
        // DISABLED: This script has been replaced by EyeAndHeadTracker.cs
    ...
    private void RequestWritePermission()
    {
        if (!Permission.HasUserAuthorizedPermission(Permission.ExternalStorageWrite))
        {
            ...
            Logger.Instance.LogInfo("Requested WRITE_EXTERNAL_STORAGE permission for JSON logging.");
        }
    }
        ...
        if (Physics.Raycast(gazePosition, gazeRotation * Vector3.forward, out RaycastHit hitInfo, 10.0f, layersToIncludeWithRay))
        {
            ...
            if (renderer != null && targetRenderers.Contains(renderer))
            {
                ...
                if (gazeOverTargetTracker >= minGazeTimeOverTarget)
                {
                    ...
                    if (destroyCount == 0)
                    {
                        sessionData.timeToFirstDestroy = timeSinceStart;
                    }
                    ...
                    sessionData.destructionEvents.Add(record);
                    ...
                    // Check if ALL objects are now destroyed → save JSON
                    if (targetRenderers.Length == 0)
                    {
                        sessionData.totalTimeToComplete = Time.realtimeSinceStartup - appStartTime;
                        sessionData.totalObjectsDestroyed = destroyCount;
                        SaveDestructionSession();
                    }
                }
            }
            else
    ...
    // === NEW: Saves the complete session log when all objects are destroyed ===
    private void SaveDestructionSession()
    {
        try
        {
            string json = JsonUtility.ToJson(sessionData, true);
            string fileName = "gaze_destruction_log.json";
            string savePath = Path.Combine(Application.persistentDataPath, fileName);

            // Guarantee the folder exists (required on ML2/Android)
            Directory.CreateDirectory(Application.persistentDataPath);

            File.WriteAllText(savePath, json);

            Logger.Instance.LogInfo($"✅ ALL {sessionData.totalObjectsDestroyed} OBJECTS DESTROYED! Log saved to: {savePath}");
            Debug.Log($"Destruction session complete.\nJSON saved to: {savePath}\n\n{json}");
        }
        catch (Exception ex)
        {
            Logger.Instance.LogError($"❌ Failed to save destruction log: {ex.Message}");
