...
namespace ARCockpit.Study
{
    ...
    public static class StatusSnapshot
    {
        ...
        public static string Build(string ip, int port)
        {
            ...
            Sb.Append(",\"wideAreaAR\":{");
            if (global::EyeAndHeadTracker.Instance != null)
            {
                Sb.Append("\"timeElapsed\":").Append(F(global::EyeAndHeadTracker.Instance.GetCurrentTrialTime()))
                  .Append(",\"destroyedCount\":").Append(global::EyeAndHeadTracker.Instance.GetTargetsDestroyed())
                  .Append(",\"remainingTargets\":[");
                var targets = global::EyeAndHeadTracker.Instance.GetRemainingTargetPositions();
                for (int i = 0; i < targets.Count; i++)
                {
                    if (i > 0) Sb.Append(",");
                    Sb.Append("{\"x\":").Append(F(targets[i].x)).Append(",\"y\":").Append(F(targets[i].y)).Append(",\"z\":").Append(F(targets[i].z)).Append("}");
                }
                Sb.Append("]");
            }
            else
            {
                Sb.Append("\"timeElapsed\":0,\"destroyedCount\":0,\"remainingTargets\":[]");
            }
            Sb.Append('}');
