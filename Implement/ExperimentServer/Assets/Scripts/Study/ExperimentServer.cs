...
namespace ARCockpit.Study
{
    ...
    public class ExperimentServer : MonoBehaviour
    {
        ...
        void Route(HttpListenerContext ctx)
        {
            ...
            switch (path)
            {
                ...
                case "/index.html":
                    ...
                    // chase a bug that is already gone. The page is a few KB from localhost; there is nothing to
                    // gain by caching it and a real cost to caching it wrong.
                ...
                // Every trial CSV in one zip, built in memory. Plain System.IO.Compression, safe off the main
                // thread. The convenience of pulling a whole session without adb.
                case "/api/files/zip":
                    SendZip(ctx);
                    return;

                // Delete files. DESTRUCTIVE, so it is POST-only and it is fenced to the trials directory the
                // same way SendFile is. The client makes the operator confirm; the server makes sure the path
                // cannot escape. Two modes: ?name=<rel> deletes one file; ?all=1 deletes every CSV under the
                // trials tree. This is a research clutter tool (clearing test runs between setup sessions), and
                // it can throw away real data, which is exactly why it says how many it removed and logs it.
                case "/api/files/delete":
                    if (ctx.Request.HttpMethod != "POST") { Send(ctx, 405, "text/plain", Encoding.UTF8.GetBytes("POST only")); return; }
                    DeleteFiles(ctx, ctx.Request.QueryString["name"], ctx.Request.QueryString["all"] == "1");
                    return;
                ...
                // logging a 404 in the console every load. Not our dashboard's fault, just noise.
                case "/favicon.ico":
        ...
        // Every CSV under the trials tree, zipped in memory. The folder structure (valid/ invalid/ etc.) is
        // preserved inside the zip so a good trial and a crashed one stay distinguishable after extraction.
        void SendZip(HttpListenerContext ctx)
        {
            try
            {
                if (!Directory.Exists(_trialsDir))
                {
                    Send(ctx, 404, "text/plain", Encoding.UTF8.GetBytes("no trials directory yet"));
                    return;
                }
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    {
                        foreach (string f in Directory.GetFiles(_trialsDir, "*.csv", SearchOption.AllDirectories))
                        {
                            string rel = f.Substring(_trialsDir.Length).TrimStart('/', '\\').Replace("\\", "/");
                ...
                ctx.Response.AddHeader("Content-Disposition", $"attachment; filename=\"trials_{stamp}.zip\"");
        ...
        void DeleteFiles(HttpListenerContext ctx, string relPath, bool all)
        {
            string trialsFull = Path.GetFullPath(_trialsDir);
            ...
            if (all)
            {
                ...
                {
                    Send(ctx, 500, "text/plain", Encoding.UTF8.GetBytes($"deleted {n} then failed: {e.Message}"));
                    return;
                }
                Debug.LogWarning($"{Tag} Mass delete: removed {n} trial CSV(s) via the dashboard.");
                SendJson(ctx, "{\"deleted\":" + n + "}");
                return;
            }

            // Single file. Resolve to a full path and require it to still be INSIDE the trials directory: that
            // defeats "../../.." however it was encoded, the same fence SendFile uses.
            string full;
            try { full = Path.GetFullPath(Path.Combine(_trialsDir, relPath ?? "")); }
            catch { full = ""; }

            bool inside = !string.IsNullOrEmpty(full) && full.StartsWith(trialsFull, StringComparison.Ordinal);
            if (!inside || !File.Exists(full))
            {
                Send(ctx, 404, "text/plain", Encoding.UTF8.GetBytes("no such file: " + relPath));
                return;
            ...
            {
                Send(ctx, 500, "text/plain", Encoding.UTF8.GetBytes("delete failed: " + e.Message));
                return;
            }
            Debug.LogWarning($"{Tag} Deleted {Path.GetFileName(full)} via the dashboard.");
            SendJson(ctx, "{\"deleted\":1}");
        }
