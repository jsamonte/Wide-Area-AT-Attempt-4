...
namespace ARCockpit.Study
{
    ...
    public class DataLogger : MonoBehaviour
    {
        ...
        static readonly string[] Columns =
        {
            ...
            "xp_connected",
            "pitch_deg", "roll_deg", "hdg_true_deg", "hdg_mag_deg", "track_deg",
            "gs_kt", "vs_fpm", "alt_m", "lat", "lon", "aoa_deg", "fpa_deg",
            "fd_mode", "fd_pitch_deg", "fd_roll_deg", "hsi_source", "gps_navid",
            "gps_rel_brg_deg", "gps_xtk_nm",
            "guid_valid", "guid_xtk_m", "guid_vdev_m", "guid_progress",
            "guid_cmd_hdg_deg", "guid_cmd_vs_fpm"
        };
        ...
        static void WriteParamHeader(StreamWriter w)
        {
            ...
            // The trial's identity, as DERIVED (never typed), plus the sim's own report of the environment it
            // was actually flown in. Both, deliberately: the plan says what the weather SHOULD be, a human sets
            // the sim up, and those can disagree. A file labeled RAIN that was flown in CLEAR is poison, and
            // this is the line that lets analysis catch it instead of averaging it in. See TrialWeatherCheck.
            SessionPlan plan = SessionPlan.Active;
            TrialSlot slot = plan?.Armed;
            if (slot != null)
            {
                w.WriteLine($"# participant = {plan.participant}, group = {plan.group}");
                w.WriteLine($"# condition = {slot.Code}, ar = {(slot.arOn ? "on" : "off")}, " +
                            $"block = {slot.block}, trial = {slot.number}, practice = {slot.practice}, " +
                            $"attempt = {slot.attempts + 1}");
                w.WriteLine($"# {TrialWeatherCheck.HeaderLine(slot)}");
            }
            else
            {
                w.WriteLine("# WARNING: no armed session slot. This trial's condition was not derived from a " +
                            "session plan, so its label is not trustworthy.");
            }
        ...
        public void WriteRow()
        {
            ...
            A(string.IsNullOrEmpty(AoiData.Current) ? "" : F(AoiData.HitDistance, 3));

            A(Bit(TrialController.XPlaneConnected));

            A(F(FlightData.PitchDeg, 3));
            A(F(FlightData.RollDeg, 3));
            A(F(FlightData.TrueHeadingDeg, 3));
            A(F(FlightData.MagneticHeadingDeg, 3));
            A(F(FlightData.TrackDeg, 3));
            A(F(FlightData.GroundspeedKt, 3));
            A(F(FlightData.VerticalSpeedFpm, 2));
            A(F(FlightData.AltitudeMeters, 2));
            A(F(FlightData.LatitudeDeg, 7));
            A(F(FlightData.LongitudeDeg, 7));
            A(F(FlightData.AngleOfAttackDeg, 3));
            A(F(FlightData.FlightPathAngleDeg, 3));

            A(FlightData.FdMode.ToString(Inv));
            A(F(FlightData.FdPitchDeg, 3));
            A(F(FlightData.FdRollDeg, 3));
            A(FlightData.HsiSourceSelect.ToString(Inv));
            A(Esc(FlightData.GpsNavId));
            A(F(FlightData.GpsRelativeBearingDeg, 3));
            A(F(FlightData.GpsCrossTrackNm, 4));

            A(Bit(GuidanceData.Valid));
            A(F(GuidanceData.CrossTrackErrorMeters, 3));
            A(F(GuidanceData.VerticalDeviationMeters, 3));
            A(F(GuidanceData.TrackProgress01, 5));
            A(F(GuidanceData.CommandedHeadingDeg, 3));
            A(F(GuidanceData.CommandedVerticalSpeedFpm, 2), last: true);
