...
namespace ARCockpit.DevSettings
{
    ...
    public class DevModeController : MonoBehaviour
    {
        ...
        /// <summary>
        /// When true, NO element dev window may be open, and none can reopen, whatever tag is shown. This is
        /// what makes 241 a trial tag rather than another 240: the cockpit anchor sets it in Deploy mode and
        /// clears it in Placement. Without it, an element window left open under 240 simply stayed open when
        /// you swapped in 241, which is why the two cockpit tags felt identical. It is a plain static so the
        /// one CockpitAnchor drives every element's controller at once, with no wiring.
        /// </summary>
        public static bool WindowsSuppressed;
        ...
        void Update()
        {
            ...
            // Trial mode (241 up): keep this element's window shut and do not let any tag reopen it. The
            // saved overrides still apply, so the element deploys exactly as tuned; only the editing UI is
            // gone, which is the whole point of a trial tag.
            if (WindowsSuppressed)
            {
                if (_window != null && _window.gameObject.activeSelf) _window.gameObject.SetActive(false);
        ...
        {
            // A trial is running (241): never open. The suppression check in Update also hides an already-open
            // one, but this stops a mode-change from opening it in the first place.
            if (show && WindowsSuppressed) return;
