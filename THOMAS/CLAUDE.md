...
- **The EEG bring-up:** see `EEG_BRINGUP.md` (the supplied Emotiv EPOC X toolkit, env + OSC verification, the
  USB-dongle gate, and the planned Unity OSC integration).
...
- **The study data capture (eye tracking, gaze AOI, trial CSV, the experiment web server):** see
  ...
  decision. **The packaged `Handoff/ExperimentServer/` folder was DELIVERED and DELETED 2026-07-17:** the
  colleague has the server running in his own project, so there is no longer a copy to keep in sync. If a
  fresh handoff is ever needed, re-package from the live files under `Assets/Scripts/Study/` per the doc; do
  not resurrect a standing copy folder (it only rots).
...
- **The ArUco tag list:** `ARUCO_MARKERS.md` is the one authoritative table (id, element, printed size, dev
  mode). Update it and the profile in the same edit whenever a tag changes.
- **The decisions record:** `DECISIONS_LOG.md` is the running log of choices that matter for the paper (study
  design, AR element design, method), with the reasoning for each. See the logging rule below.
...
- **Log important decisions as they are made.** When I settle something that matters for the paper (study
  design, AR element design, method, a scoping call), record it in `DECISIONS_LOG.md` with its reasoning, in the
  same session I decide it. I am not good at keeping a methods record myself, so you managing this is how it gets
  captured. Keep superseded decisions in place, marked Superseded, do NOT delete them: the point is that the
  evolution is legible later. Things will change once we are in the sim room; log the change, keep the old.
...
## Out-of-lab autonomous work (the two-context workflow)

There are two contexts: OUT of lab (no ML2, most hours, plan and build) and IN lab (ML2 present, scarce,
test and give feedback). The whole point is to make lab time pure testing: never spend it waiting on a
change or working out a plan. So we front-load all the planning and coding out of lab, and the lab session
is walking a checklist.

The hard reality: I cannot run Unity or verify on device here, and build/deploy is yours (see the guardrail).
So any change I make out of lab is unverified until you are at the headset. That is the risk to manage: a
batch of plausible-looking edits that turn out broken and cannot be caught until the next lab day. These
rules exist to shrink that risk, not pretend it away.

**Work on main. No feature branches or worktrees.** A fuller isolated-branch + CI pipeline was sketched once
but is NOT stood up, and Thomas did not sign off on it here: do not create branches, do not leave work on a
side branch for review, just edit on main. (Commit only when Thomas asks, per the git guardrail.) If some
change genuinely needs isolation, ASK first rather than branching by default.

- **Every out-of-lab change lands with a device-test note.** Because I cannot verify on device, each change
  ships with an explicit "here is how you confirm this on the ML2" checklist: what to look at, what correct
  looks like, what would mean it is broken. These accumulate in `LAB_TEST_QUEUE.md` (the pending-device-test
  queue). Your lab session is: build, put it on the headset, walk that queue, give feedback, I clear passed
  items and fix failed ones. No planning or coding in the lab.
- **Batch to what one device session can clear.** Do not queue 20 changes when a session can test 6. Unverified
  changes stack into debt that is invisible until it breaks. Keep each batch clearable in a single lab visit,
  and order the queue so the highest-risk / most-blocking items get tested first.
- **Flag device-only risk out loud.** When a change can ONLY be verified on device (a shader look, a marker
  pose, controller input, anything visual or perception-driven), say so and treat it as higher risk. Prefer,
  where possible, factoring logic so it CAN be exercised in edit mode. Anything with edit-mode test coverage
  is far safer than anything that is device-only.
- **Plan the batch before building it.** Same "plan before code" rule, scaled up: agree the set of changes and
  each one's spec + device-test note with Thomas before a big out-of-lab push, so I am executing an approved
  plan, not improvising unverifiable changes while he is away.
...
- **Dev settings overlay (on-device tuning).** A live, marker-driven dev window
  ...
  `x2` = pure Unity-authored values. So map 210/211/212 and NavBall 220/221/222 pick BOTH the printed size
  (by the ones-digit size rule above) AND the dev mode: the two are set together in each profile's preset
  table keyed by the tag id, so size is NOT a runtime dev-window tunable (it is fixed per printed tag).
  Profiles expose their other tunables via `IDevTunableSource` (lambdas bound to
...
* **Next prompt (after `/clear`):** [ONLY when the plan is to `/clear` and keep going on a known next item
  (a queued batch, a one-item-per-clear run). Give the literal prompt I can paste to start the next item, in a
  code block, self-contained enough to stand with no memory of this conversation (name the item and where its
  spec lives, e.g. "Do item B (map track mode) from TODO.md"). OMIT this line entirely when we are mid-task, when
  there is no obvious next item, or when the next step is mine to decide.]
